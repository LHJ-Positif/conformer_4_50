...
class AV_SELD(nn.Module):
    ...
    def train_model(self, dl_train, epoch, ckpt_dir):
        ...
        for batch_idx, (audio_features, visual_features, labels, initial_time, sequence) in enumerate(tqdm(dl_train, desc=f'Train {epoch}', dynamic_ncols=True,)):
            ...
            if conf.training_param['wandb_ok']:
                wandb.log({"train/iter_loss": loss})
                
            torch.save(self.model.state_dict(), ckpt_dir / 'model_{:03d}.ckpt'.format(epoch))

            #if batch_idx == 40: # for debugging
            #    break
        # 에폭이 끝난 후 현재 learning rate 계산
        current_lr = self.opt.param_groups[0]['lr']
        
        if conf.training_param['wandb_ok']:
            wandb.log({
                "train/epoch_loss": training_loss / batch_idx,
                "train/epoch_learning_rate": current_lr
                # "train/learning_rate": self.scheduler.get_last_lr()[0]  # 리스트의 첫 번째 요소만 로깅
                })
